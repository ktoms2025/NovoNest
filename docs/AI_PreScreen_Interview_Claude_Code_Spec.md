...
**Prototype spec, not a committed product requirement**
Working title used internally: "Linea Interview" (named but unconfirmed in final eSkill Product Strategy v2.1, flagged there as needing market research and customer validation before roadmap confirmation)
...
**v2 update (Aug 20, 2026):** Section 3, Steps 1 and 2 revised after live testing of the Phase 1 build surfaced a problem: the interview was exposing internal scores and field names directly to the candidate, and reading as a mechanical Q&A form rather than a conversation. See the callout boxes in those sections for what changed and why.
...
All names, scores, and transcript content below are fabricated for this prototype. None of it represents a real candidate or real eSkill assessment output. Shape and field names follow the structure already established in the Behavioral Assessment PRD (Big Five, Integrity, Impression Management, RIASEC) and the AI Sample MVP scoring approach (per-scenario rubric scores), so swapping in real data later should be a drop-in replacement rather than a rebuild.

```json
{
  "role": {
    "title": "Call Center Agent — General/BPO",
    "req_id": "CC-1042",
    "blueprint_dimensions_required": [
      "Skills", "Behavioral", "Personality", "Cognitive", "Trainability"
    ]
  },
  "candidates": [
    {
      ...
      "hard_skills": {
        "phone_email_etiquette": 88,
        "data_entry_accuracy": 74,
        "multitasking_typing": 81,
        "troubleshooting_logic": 69
      },
      "personality": {
        "big_five": {
        ...
        "integrity": 85,
        "impression_management": 52,
        "riasec_top_codes": ["Conventional", "Social", "Enterprising"]
      },
      "ai_sample": {
        "status": "complete",
        "scenarios": [
          {
            "name": "Angry customer escalation call",
            "score": 82,
            "rubric_notes": "Strong de-escalation language, maintained tone under pressure"
          },
          {
            "name": "Multitasking order lookup",
            "score": 75,
            "rubric_notes": "Accurate but slower than target pace under time pressure"
          }
        ]
      },
      "blueprint_coverage": {
    ...
    },
    {
      ...
      "hard_skills": {
        "phone_email_etiquette": 70,
        "data_entry_accuracy": 65,
        "multitasking_typing": 68,
        "troubleshooting_logic": 60
      },
      "personality": {
        "big_five": {
        ...
        "integrity": 79,
        "impression_management": 61,
        "riasec_top_codes": ["Social", "Enterprising", "Conventional"]
      },
      "ai_sample": {
        "status": "complete",
        "scenarios": [
          {
            "name": "Angry customer escalation call",
            "score": 63,
            "rubric_notes": "Polite but generic responses, limited de-escalation technique"
          },
          {
            "name": "Multitasking order lookup",
            "score": 58,
            "rubric_notes": "Accuracy dropped noticeably under simulated time pressure"
          }
        ]
      },
      "blueprint_coverage": {
    ...
    },
    {
      ...
      "hard_skills": {
        "phone_email_etiquette": 93,
        "data_entry_accuracy": 89,
        "multitasking_typing": 90,
        "troubleshooting_logic": 84
      },
      "personality": {
        "big_five": {
        ...
        "integrity": 90,
        "impression_management": 55,
        "riasec_top_codes": ["Conventional", "Investigative", "Social"]
      },
      "ai_sample": {
        "status": "complete",
        "scenarios": [
          {
            "name": "Angry customer escalation call",
            "score": 89,
            "rubric_notes": "Consistently calm, used structured de-escalation steps"
          },
          {
            "name": "Multitasking order lookup",
            "score": 87,
            "rubric_notes": "Fast and accurate, handled interruption well"
          }
        ]
      },
      "blueprint_coverage": {
...
> **Critical: the rationale is internal-only.** The output must keep two fields strictly separate: a `candidate_facing_question` (natural, conversational, contains no scores, numbers, or internal field names) and an `internal_rationale` (the scoring/gap logic behind the question, for the recruiter's report). Live testing of Phase 1 showed the model surfacing raw scores and field names like "troubleshooting_logic score came in at 69" directly to the candidate. That must never happen. The candidate should never see or infer that a specific number drove a specific question.
...
> **Conversational requirements (added after Phase 1 testing):**
> - No visible "Question X of Y" labeling and no exposed scores. The candidate should experience this as a conversation with a recruiter, not a form or a test.
> - Each AI turn should briefly acknowledge or react to what the candidate just said before moving on, the way a person naturally would. No jumping straight from one scripted prompt to the next.
> - Target roughly a 10-minute conversation overall. Build in a warm opening/practice turn, a natural (not rigidly fixed) number of questions with light conversational transitions between them, and a short closing turn. The AI should track approximate pacing and wrap up gracefully rather than mechanically stopping at a fixed question count.
> - Tone: warm, low-pressure, conversational, like a real recruiter doing a pre-screen call. All analysis and scoring happens after the conversation, in the report, never signaled to the candidate during it. The goal is to avoid candidates dropping off mid-interview because they feel like they're being tested rather than talked to.
...
- Whether the AI Recruiter's conversational tone (acknowledgment, pacing, warmth) needs its own review standard, separate from rubric/scoring review, since an interview that feels cold or test-like could itself suppress candidate completion rates independent of scoring accuracy
...
**Phase 1 — Core loop, text only**
Mock candidate data → question generation → chat-based interview (no voice yet) → grading → report screen. Prove the report is genuinely useful before adding voice complexity.

**Phase 2 — Add voice**
Layer in ElevenLabs for the interview delivery step. Keep chat as a fallback path.

**Phase 3 — Connect to the existing POC concept**
Wire the report's recommendation into the same Progress/Waitlist/Reject action pattern already mocked up in the Candidate 360 and Compare Candidates POC screens, so the two prototypes tell one coherent story when shown together.
